  ...
  public class SacnReceiver : IDisposable, ISacnReceiver
  {
    ...
    public void JoinMulticastGroup(UInt16 universe, bool ipv6 = false)
    {
      ...
      else
      {
        ...
        {
          if (nic.OperationalStatus != OperationalStatus.Up || !nic.SupportsMulticast || !nic.Supports(NetworkInterfaceComponent.IPv4))
            continue;
          ...
          foreach (var ua in nic.GetIPProperties().UnicastAddresses)
          {
            ...
            try
            {
              Socket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.AddMembership, new MulticastOption(multicastAddress, ua.Address));
            }
            catch (Exception)
            {
            }
      ...
      else
      {
        ...
        foreach (var nic in NetworkInterface.GetAllNetworkInterfaces())
        {
          ...
          foreach (var ua in nic.GetIPProperties().UnicastAddresses)
          {
            ...
            try
            {
              Socket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.DropMembership, new MulticastOption(multicastAddress, ua.Address));
            }
            catch (Exception)
            {
            }
